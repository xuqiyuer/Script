...
*
    ...
    Thanks:
       whyour大佬，TG：https://t.me/joinchat/O1WgnBbM18YjQQVFQ_D86w
    
    Quantumult X:
    [task_local]
    10 * * * * https://raw.githubusercontent.com/MoPoQAQ/Script/main/Me/jx_cfd.js, tag=京喜财富岛, img-url=https://raw.githubusercontent.com/MoPoQAQ/Script/main/icons/jxcfd.png, enabled=true
    Loon:
    [Script]
    cron "10 * * * *" script-path=https://raw.githubusercontent.com/MoPoQAQ/Script/main/Me/jx_cfd.js,tag=京喜财富岛
    Surge:
    京喜财富岛 = type=cron,cronexp="10 * * * *",wake-system=1,timeout=20,script-path=https://raw.githubusercontent.com/MoPoQAQ/Script/main/Me/jx_cfd.js
*
**/
...
function getUserInfo() {
  return new Promise(resolve => {
    $.get(taskUrl('user/QueryUserInfo'), async (err, resp, data) => {
      try {
        //$.log(data);
        const { iret, SceneList, sErrMsg, strMyShareId , strPin } = JSON.parse(data);
        $.log(`\n获取用户信息：${sErrMsg}\n${$.showLog ? data : ''}`);     
        var sceneId = eval('(' + JSON.stringify(SceneList) + ')')['1001'].dwSceneId;        
        //$.log(sceneId);
        $.info = {
          ...$.info,
          sceneId,
...
//领取钞票
// GET /jxcfd/user/GetMoney?strZone=jxcfd&bizCode=jxcfd&source=jxcfd&dwEnv=7&_cfd_t=1606996211523&ptag=138631.26.55&dwSceneId=1001&strEmployeeId=undefined&dwSource=1&strZone&_ste=1&_=1606996211525&sceneval=2&g_login_type=1&callback=jsonpCBKK&g_ty=ls HTTP/1.1
//{"RedPack":{"dwRedPackNum":0},"dwCurTime":1607001061,"dwExpericnce":448,"dwMoney":448,"iRet":0,"sErrMsg":"success","strPin":"jd_5a3d9cc737f52"}
function getMoney(){
  return new Promise(async resolve => {
    $.get(
      taskUrl('user/GetMoney', `dwSceneId=${$.info.sceneId}&strEmployeeId=undefined&dwSource=1`),
      async (err, resp, data) => {
        try {
          //$.log(data);
          const {
...
//捡地上的奖励
//GET /jxcfd/consume/TreasureHunt?strZone=jxcfd&bizCode=jxcfd&source=jxcfd&dwEnv=7&_cfd_t=1607001756266&ptag=138631.26.55&strIndex=small_stone&dwIsShare=0&_=1607001756272&sceneval=2&g_login_type=1&callback=jsonpCBKY&g_ty=ls HTTP/1.1
...
function doTreasureHunt(place){
  return new Promise(async resolve => {    
    $.get(taskUrl('consume/TreasureHunt', `strIndex=${place}&dwIsShare=0`), async (err, resp, data) => {
      try {
        //$.log(data);
        const {iRet, dwExpericnce, sErrMsg } = JSON.parse(data);
        $.log(`\n状态：${sErrMsg} 获取随机奖励：${dwExpericnce}元\n${$.showLog ? data : ''}`);
      } catch (e) {
        $.logErr(e, resp);
      } finally {
        resolve();
  ...
  return new Promise(resolve => {
    $.get(taskListUrl(`Award`,`taskId=${taskId}`), (err, resp, data) => {
      try {
        ...
        $.log(`\n${taskName}[领奖励]：${msg.indexOf('活动太火爆了') !== -1 ? '任务进行中或者未到任务时间' : msg}：获得钞票 ${prizeInfo.ddwMoney}\n${$.showLog ? data : ''}`,);
        resolve(ret === 0);
      } catch (e) {
        $.logErr(e, resp);
      } finally {
